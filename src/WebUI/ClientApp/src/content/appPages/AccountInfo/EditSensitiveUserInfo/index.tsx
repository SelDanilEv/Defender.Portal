...
const EditSensitiveUserInfo = (props: any) => {
  ...
  const sendVerificationCode = () => {
    APICallWrapper({
      ...
      utils: u,
      showSuccess: true,
      successMesage: u.t("personal_info_page__notification_proceed"),
    } as APICallProps);
  };
  ...
  const ProceedButtonName = "proceed";
  ...
  const {
    ...
    [ProceedButtonName]: ButtonStatusDisabled,
    [SaveButtonName]: ButtonStatusHided,
    [BackButtonName]: ButtonStatusHided,
  });

  const UpdateButtonsStatus = () => {
    if (isButtonVisible(ProceedButtonName)) {
      if (
        sessionUser?.email != request?.email ||
        (request.password && request.password.length > 2)
      ) {
        updateButtonStatus(ProceedButtonName, ButtonStatusDisplayed);
      } else {
        updateButtonStatus(ProceedButtonName, ButtonStatusDisabled);
      }
    } else {
  ...
  const proceedHandler = () => {
    ...
    updateButtonStatus(ProceedButtonName, ButtonStatusHided);
  };

  // ----- buttons -----

  const RenderVerificationCodeSection = () => {
    if (isButtonVisible(ProceedButtonName)) return;
  ...
  const RenderButtons = () => {
    return (
      <>
        {isButtonVisible(ProceedButtonName) ? (
          <Grid container justifyContent="flex-end">
            <Button
              disabled={isButtonDisabled(ProceedButtonName)}
          ...
          <Grid container spacing={2} sx={{ fontSize: "1.2em" }}>
            ...
            <Grid item xs={12} sm={6} md={7}>
              <TextField
                id="email"
                sx={{ padding: 0 }}
                disabled={!isButtonVisible(ProceedButtonName)}
