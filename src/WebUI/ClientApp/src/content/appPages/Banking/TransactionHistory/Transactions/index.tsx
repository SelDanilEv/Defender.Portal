...
import { PagedRequest } from "src/models/base/PagedRequest";
import { CurrentPagination } from "src/models/base/CurrentPagination";

const HistoricalTransactions = (props: any) => {
  const u = useUtils();

  const [transactions, setTransaction] = useState<Transaction[]>([]);

  const [paginationRequest, setPaginationRequest] = useState<PagedRequest>({
    page: 0,
    pageSize: 5,
  } as PagedRequest);
  ...
  const reloadTransactionHistory = () => {
    const url =
      ...
      `?CurrentWalletNumber=${props.wallet.walletNumber}` +
